...
//    Matrix& operator    =   (const SpMatrixXd& m);
    ...
    Matrix row(int i);
    Matrix col(int j);
//    Eigen::RowXpr Matrix::row(int I);
//    Eigen::ColXpr Matrix::col(int I);
//    Eigen::ConstRowXpr Matrix::row(int I) const;
//    Eigen::ConstColXpr Matrix::col(int I) const;
...
Matrix::Matrix(const MatrixXd& m):dx(m.rows()),dy(m.cols())
{
    p=new MatrixXd(m);
...
Matrix operator * (const Matrix& m1,const Matrix& m2)
{
    ...
    return Matrix(m1.p->array()*m2.p->array());
}
Matrix operator * (double c,const Matrix& m2)
{

    return Matrix(m2.P()*c);
}
...
Matrix operator + (double c,const Matrix& m)
{
    ...
    return c*Matrix::ones(m.dx,m.dy)+m;
}
...
Matrix operator / (double c,const Matrix& m)
{
    ...
    return Matrix((m/c).p->cwiseInverse());
}
...
Matrix operator ^ (const Matrix& m, double c)
{
    ...
    if(c>0)
    {
        ...
        return Matrix((*m.p).array().pow(c));

    }
    else
    {
        return Matrix((*m.p).array().pow(c).cwiseInverse());
    }
...
Matrix operator + (double c,const Matrix&& m)
{
    ...
    return m+c*Matrix::ones(m.dx,m.dy);
}
Matrix operator + (const Matrix&& m, double c)
{
   return (c+m);
...
void Matrix::remove_nan(double x)
{
    ...
    for(int j=0;j<dx;j++)
        for(int i=0;i<dy;i++)
            if(std::isnan((*p)(j,i))||std::isinf((*p)(j,i))) (*p)(j,i)=x;
}
...
Matrix Matrix::row(int I)
{
    ...
    return Matrix(p->row(I));
}

Matrix Matrix::col(int I)
{

    return Matrix(p->col(I));
}
//Eigen::RowXpr Matrix::row(int I)
//{
//    return p->row(I);
