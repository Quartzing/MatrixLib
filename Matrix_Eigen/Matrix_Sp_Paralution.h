...
#include "paralution-0.7.0/src/paralution.hpp"

using namespace std;
using namespace paralution;

typedef double ValueType;
LocalMatrix<ValueType> SpMat ;
LocalVector<ValueType> SpVec ;
enum Sp_Type{COO,CSR,MCSR,ELL,DIA,HYB,DENSE};
...
private:
    ...
    void allocArrays()
    {
        ...
        {
        case COO:
            p->AllocateCOO("COO Matrix",nnz,dx,dy);
            break;
        case CSR:
            p->AllocateCSR("CSR Matrix",nnz,dx,dy);
            break;
        case ELL:
            p->AllocateELL("ELL Matrix",nnz,dx,dy);
            break;
        case DIA:
            p->AllocateDIA("DIA Matrix",nnz,dx,dy);
            break;
        case HYB:
            p->AllocateHYB("HYB Matrix",nnz,dx,dy);
            break;
        }
...
///DEFULT CONSTRUCTOR
Matrix_Sp::Matrix_Sp() : dx(1),dy(1),nnz(1) {}
///CONSTRUCTOR
Matrix_Sp::Matrix_Sp(int sizeX,int sizeY,int NNZ):dx(sizeX),dy(sizeY),nnz(NNZ)
{
    allocArrays();
}

///COPY CONSTRUCTOR
Matrix_Sp::Matrix_Sp(const Matrix_Sp& m):dx(m.dx),dy(m.dy),nnz(m.nnz)
{
    allocArrays();
    (*p)=(*m.p);
}
